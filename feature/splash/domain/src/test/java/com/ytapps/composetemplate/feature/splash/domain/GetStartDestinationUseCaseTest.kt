...
internal class GetStartDestinationUseCaseTest {
    ...
    @Test
    fun `given user has account when invoke then return Home destination`() {
        coEvery { splashRepository.hasUser() } returns true

        val result = runBlocking { getStartDestinationUseCase() }

        Truth.assertThat(result).isEqualTo(SplashDestination.Home)
    }

    @Test
    fun `given user has no account when invoke then return Login destination`() {
